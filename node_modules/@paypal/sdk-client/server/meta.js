...
function validateWebSDKUrl({
  ...
  if (pathname !== _sdkConstants.WEB_SDK_BRIDGE_PATH) {
...
function isWebSDKUrl(hostname, pathname) {
  if (isSDKUrl(hostname) && pathname === _sdkConstants.WEB_SDK_BRIDGE_PATH) {
    return true;
  }
  return false;
}
...
function sanitizeSDKUrl(sdkUrl) {
  ...
  if (isLegacySDKUrl(url.hostname, url.pathname) || isWebSDKUrl(url.hostname, url.pathname)) {
    url.search = "";
    url.hash = "";
    return url.toString();
  }
